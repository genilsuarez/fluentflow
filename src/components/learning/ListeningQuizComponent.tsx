...
const ListeningQuizComponent: React.FC<ListeningQuizComponentProps> = ({ module }) => {
  ...
  useEffect(() => {
    ...
    const handleKeyPress = (e: KeyboardEvent) => {
      ...
      else if (e.key === ' ' && !showResult) { e.preventDefault(); playQuestion(); }
    };
    window.addEventListener('keydown', handleKeyPress);
    return () => window.removeEventListener('keydown', handleKeyPress);
  }, [showResult, currentQuestion, processedQuestions.length, handleAnswerSelect, handleNext, playQuestion]);
  ...
  return (
    <div className="quiz-component__container">
      <LearningProgressHeader
        ...
        mode="listening-quiz"
        helpText={showResult ? t('learning.pressEnterNext') : 'Listen and choose the correct answer'}
      />
      ...
      <div className="quiz-component__question-card">
        {ttsAvailable ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.75rem', padding: '1rem 0' }}>
            ...
            <span style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--theme-text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              {isPlaying ? 'Playing…' : 'Tap to listen'}
            </span>
          </div>
        ) : (
          // Fallback: show text if TTS not available
          <h3 className="quiz-component__question-title dynamic-text-color" style={{ '--dynamic-text-color': textColor } as React.CSSProperties}>
            <ContentRenderer content={ContentAdapter.ensureStructured(currentQuestion?.question || currentQuestion?.sentence || '', 'quiz')} />
          </h3>
        )}

        {/* Show the text after answering for learning */}
        {showResult && (
          <p style={{ fontSize: '0.85rem', fontWeight: 500, color: textColor, textAlign: 'center', marginTop: '0.5rem', opacity: 0.85 }}>
        ...
        <div ref={optionsRef} className="quiz-component__options">
          {(currentQuestion?.options || []).map((option, index) => {
            ...
            if (showResult) {
              if (currentQuestion?.options[index] === currentQuestion?.correct) buttonClass += ' quiz-component__option--correct';
              else if (index === selectedAnswer && currentQuestion?.options[index] !== currentQuestion?.correct) buttonClass += ' quiz-component__option--incorrect';
              ...
              <button key={index} onClick={() => handleAnswerSelect(index)} disabled={showResult} className={buttonClass}>
                <div className="quiz-component__option-content">
                  <div className="quiz-component__option-left">
                    ...
                    <span className="quiz-component__option-text dynamic-text-color" style={{ '--dynamic-text-color': textColor } as React.CSSProperties}>
                      <ContentRenderer content={ContentAdapter.ensureStructured(option, 'quiz')} />
                    </span>
                  </div>
                  {showResult && (
                    <div>
                      {currentQuestion?.options[index] === currentQuestion?.correct && <CheckCircle className="quiz-component__option-icon quiz-component__option-icon--correct" />}
                      {index === selectedAnswer && currentQuestion?.options[index] !== currentQuestion?.correct && <XCircle className="quiz-component__option-icon quiz-component__option-icon--incorrect" />}
                    </div>
                  )}
                </div>
              </button>
        ...
        {showResult && currentQuestion?.explanation && (
          <div className="quiz-component__explanation">
            <ContentRenderer content={ContentAdapter.ensureStructured(currentQuestion.explanation, 'explanation')} />
          </div>
        )}
      </div>
      ...
      <div className="game-controls">
        ...
        <button onClick={handleNext} disabled={!showResult} className="game-controls__primary-btn game-controls__primary-btn--green">
          <span>{currentIndex < processedQuestions.length - 1 ? t('learning.nextQuestion') : t('learning.finishQuiz')}</span>
