...
export const CACHE_NAME = 'fluentflow-v3';
...
/**
 * Fetch the asset manifest and return absolute URLs for all app chunks.
 * Falls back to parsing index.html if manifest is unavailable.
 */
async function getAppAssetUrls(): Promise<string[]> {
  const base = getBaseUrl();
  const origin = typeof window !== 'undefined' ? window.location.origin : '';

  try {
    // Try asset-manifest.json first (generated by build plugin)
    const manifestUrl = `${origin}${base}asset-manifest.json`;
    const res = await fetch(manifestUrl);
    if (res.ok) {
      const assets: string[] = await res.json();
      return assets.map(a => `${origin}${base}${a}`);
    }
  } catch {
    // Manifest not available, fall through
  }

  // Fallback: parse index.html for script/link tags
  try {
    ...
    // Match script src and link href pointing to /assets/
    const srcPattern = /(?:src|href)=["']([^"']*\/assets\/[^"']+)["']/g;
    let match;
    while ((match = srcPattern.exec(html)) !== null) {
      const url = match[1].startsWith('http')
        ? match[1]
        : `${origin}${match[1].startsWith('/') ? '' : base}${match[1]}`;
