...
export async function fetchProgress(): Promise<RemoteProgressRow[] | null> {
  ...
  // frío (el cliente todavía restaurando el token) — devolver [] hacía que
  // downloadOnLogin() lo confundiera con "sin datos remotos" y marcara
  // cloudHydrated=true sin haber pedido nada, dejando el dispositivo pegado
  // en su copia local vieja (ver auditoría House & Rooms, multi-navegador).
...
/**
 * Revisión monotónica del usuario (migración 026 de LearnBackend,
 * sync_cursor). Un solo número que sube en cada escritura real a
 * progress/activity_events — el cliente lo compara contra el último que vio
 * para decidir "¿hace falta pull?" sin ambigüedad, en vez de las heurísticas
 * (downloaded, cloudHydrated, hasLocalStatsCache) que hoy deciden eso y se
 * desincronizan entre sí. Todavía no se llama desde ningún lado (fase 2 del
 * plan de sync).
 */
export async function fetchSyncRevision(): Promise<number | null> {
  const {
    data: { session },
  } = await supabase.auth.getSession();
  if (!session?.user) return null;
